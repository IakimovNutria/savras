...
import {getUserPipelines} from "../../store/main-reducer/selectors";
import {getSharedPipelines} from "../../store/main-reducer/selectors";
    ...
    return (
        <React.Fragment>
            <header className="pipeline__header">
                <h1>{pipeline.name}</h1>
                {
                    canEdit && <button onClick={() => setVisible(!visible)} className="pipeline__header-button">Create</button>
                }
            ...
            {
                pipeline.cells.map((cellInfo) => (<Cell cellInfo={cellInfo} pipelineId={id} canEdit={canEdit}/>))
            }
        </React.Fragment>);
}
