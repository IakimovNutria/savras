...
function CellOutputs({cellId, outputs, updateOutputNameHandler, saveFilesHandler, cellParams, updateShowGraphHandler}: OutputsParams): JSX.Element {
            ...
            <ul className="cell__output-items">
                {
                    outputNames.map((output) => {
                        return (
                            <li key={cellId + output} className="cell__output-item">
                                <div className="cell__show-graph-checkbox">
                                    <img alt="graph-icon" src="/img/graph-icon.png" width={15} height={15}/>
                                    <input type="checkbox" checked={isShowGraph[output]}
                                           id={output} onChange={updateShowGraphHandler}/>
                                </div>
                                <span>{output}:</span>
                                <input value={names[output]} className="cell__text-input"
                                       type="text" id={output} onChange={updateOutputNameHandler}/>
                            </li>);
                    })
                }
            </ul>
            <button className="cell__save-button" onClick={saveFilesHandler}>Save output files</button>
        </div>);
}
