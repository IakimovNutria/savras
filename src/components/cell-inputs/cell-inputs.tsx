...
function CellInputs({cellId, updateInputHandler, updateColumnHandler, submitInputsHandler, updateShowGraphHandler, cellParams}: InputProps): JSX.Element {
    ...
    return (
        <div className="cell__inputs">
            ...
            <ul className="cell__input-items">
                {
                    inputsArray.map((input) => (
                        <li className="cell__input-item" key={cellId + input.name}>
                            <div className="cell__show-graph-checkbox">
                                <img alt="graph-icon" src="/img/graph-icon.png" width={15} height={15}/>
                                <input type="checkbox" checked={isShowGraph[input.name]}
            ...
            <button className="cell__save-button" onClick={submitInputsHandler}>Save inputs</button>
        </div>);
}
